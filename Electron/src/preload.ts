    ...
    async.each(files, async (file, callback) => {
      ...
      async.each(files, async (file, callback) => {
        ...
        let getMBits = () => {
          ...
          let mbits = roundDecimalPlace(difference / 1000 / 1000, 2);
        ...
        emitter.on("progress", args => {
          ...
          if(percent != undefined) {
            ...
            let total = roundDecimalPlace(totalDownloaded / totalSize * 100, 2);
...
function saveDialog(fileName? : string) {
...
function getSize(url : string) : Promise<number | undefined> {
...
function roundDecimalPlace(Number : number ,place : number) {
